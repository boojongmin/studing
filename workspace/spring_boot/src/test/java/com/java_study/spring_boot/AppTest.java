package com.java_study.spring_boot;
...
import com.java_study.spring_boot.config.ProductionConfiguration;
import com.java_study.spring_boot.config.SampleController;
...
@SpringApplicationConfiguration(classes = ProductionConfiguration.class)
@ActiveProfiles("test")
@WebAppConfiguration
...
//
//		given().multiPart(file).when().post("/fileUpload").then()
//				.body("size", greaterThan(10)).body("name", equalTo("file"));
//	}
...
//		IOUtils.write("Something21", new FileOutputStream(file));
//
//		given().multiPart("controlName", "original", new FileInputStream(file))
//				.when().post("/fileUpload2").then()
...
//		given().multiPart("controlName", "original", "something32".getBytes())
//				.when().post("/fileUpload2").then()
//				.body("size", greaterThan(10))
//				.body("name", equalTo("controlName"))
//				.body("originalName", equalTo("original"));
...
//		given().multiPart("controlName", "original", "something32".getBytes(),
//				"mime-type").when().post("/fileUpload2").then()
//				.body("size", greaterThan(10))
//				.body("name", equalTo("controlName"))
//				.body("originalName", equalTo("original"))
...
//
//		given().multiPart("controlName1", "original1",
//				"something123".getBytes(), "mime-type1")
//				.multiPart("controlName2", "original2",
//						new FileInputStream(file), "mime-type2").when()
//				.post("/multiFileUpload").then().root("[%d]")
...
//				.multiPart("controlName2", greeting, "application/json").when()
//				.post("/multiFileUpload").then().root("[%d]")
...
//				.body("content", withArgs(1), notNullValue()).extract()
//				.path("[1].content");
//
...
//		given().multiPart("controlName", "original", "something32".getBytes(),
//				"mime-type").param("param", "paramValue").when()
//				.post("/fileUploadWithParam").then().root("file")
//				.body("size", greaterThan(10))
//				.body("name", equalTo("controlName"))
//				.body("originalName", equalTo("original"))
//				.body("mimeType", equalTo("mime-type")).noRoot()
